...
// Root route for MCQ Test operations
router.get('/', (req, res) => {
  res.render('mcqTest/index'); // Or use res.send('MCQ Test Home')
});

// API: Get all tests as JSON
router.get('/api/tests', async (req, res) => {
  try {
    const tests = await McqTest.find();
    res.json(tests);
...
// ✅ GET route to render the Add Test form
router.get('/add', (req, res) => {
  res.render('mcqTest/add'); // Looks for views/mcqTest/add.ejs
});

// ✅ Create a new test with correctOption field
router.post('/add', async (req, res) => {
  try {
    ...
    const newTest = new McqTest({ testName, questions: formattedQuestions });
    await newTest.save();

    res.redirect('/mcqTest/view');
  } catch (error) {
...
// View all submissions (admin panel)
router.get('/submissions', async (req, res) => {
  try {
    const submissions = await Submission.find().populate('testId');
    res.render('mcqTest/submissions', { submissions });
  } catch (error) {
